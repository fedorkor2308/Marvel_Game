...
export default function CoinFlip({ onComplete, autoResult = null }) {
    const [phase, setPhase] = useState(autoResult ? STATES.FLIPPING : STATES.IDLE)
    const [result, setResult] = useState(null)

    // When autoResult is provided (server decided), animate then show result
    useState(() => {
        if (autoResult) {
            setTimeout(() => {
    ...
    const handleToss = () => {
        if (autoResult) return  // server-decided, no manual toss
